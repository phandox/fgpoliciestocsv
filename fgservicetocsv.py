...
option_0 = {'name': ('-i', '--input-file'), 'help': '<INPUT_FILE>: Fortigate configuration file. Ex: fgfw.cfg',
            'nargs': 1}
option_1 = {'name': ('-o', '--output-file'), 'help': '<OUTPUT_FILE>: output csv file (default \'./service-out.csv\')',
            'default': 'service-out.csv', 'nargs': 1}
...
# -- VIP settings
p_service_setting = re.compile('^\s*set\s+(?P<service_key>\S+)\s+(?P<service_value>.*)$', re.IGNORECASE)
        ...
        for line in fd_input:
            ...
            if in_vip_block:
                if p_service_name.search(line):
                    ...
                    address_elem['name'] = service_name
                    if not ('name' in order_keys): order_keys.append('name')

                # We match a setting
                if p_service_setting.search(line):
                    service_key = p_service_setting.search(line).group('service_key')
                    ...
                    service_value = p_service_setting.search(line).group('service_value').strip()
                    service_value = re.sub('["]', '', service_value)
                    # if service_key == "subnet":
                    #     ip_addr, subnet_mask = divide_subnet(service_value)
                    #     address_elem['ip_addr'] = ip_addr
                    #     address_elem["subnet_mask"] = subnet_mask

                    address_elem[service_key] = service_value
